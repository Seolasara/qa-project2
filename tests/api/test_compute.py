...
class TestComputeCRUD:
    ...
    def test_VM011_get_vm_one(self, api_headers, resource_factory, base_url_compute):
        # payload를 이용하여 resource_factory가 vm 생성
        payload = {"name": f"team2-{uuid.uuid4().hex[:6]}",
            "zone_id": "0a89d6fa-8588-4994-a6d6-a7c3dc5d5ad0",
        ...
        vm_id = new_vm["id"]
        vm_name = new_vm["name"]       # 리소스 생성할 때 name값도 반환하도록 픽스처 설정했습니다.

        # 조회 테스트 <<<< 여기서부터 테스트가 실행됩니다!!
        url = f"{base_url_compute}/virtual_machine/{vm_id}"
        response = requests.get(url, headers=api_headers)
        assert response.status_code == 200, f"⛔ [FAIL] 조회 실패 - {response.status_code}: {response.text}"
        # 응답 바디 검증
        response_vm = response.json()
        assert isinstance(response_vm, dict) 
        assert response_vm["id"] == vm_id
        assert response_vm["name"] == vm_name
        assert response_vm["zone_id"] is not None
